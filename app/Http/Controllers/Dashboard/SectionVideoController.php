...
class SectionVideoController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return response()->json(getModelData(model: new CourseVideo(),relations: ['course' => ['id', 'title_ar','title_en' ],'section' => ['id', 'title_ar','title_en' ]]));
        } else {
            // Return the main view with data
            return view('dashboard.videos.index', compact( 'visited_site','courses','quizzes'));
        }
    }
    public function getvideosbyclasses(Request $request,$sectionId)
    {
          $this->authorize('view_videos');
        ...
        if ($request->ajax()) {
            // Return JSON data for AJAX requests
            return response()->json(getModelData(model: new CourseVideo(), andsFilters: [['section_id', '=', $sectionId]],relations: ['course' => ['id', 'title_ar','title_en' ],'section' => ['id', 'title_ar','title_en' ]]));
        } else {
            // Return the main view with data
